...
export default function SupplierOnboardingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (user) {
        // Récupère le profil fournisseur lié à l'utilisateur
        const { data: supplier } = await supabase
          .from('suppliers')
          .select('id, wilaya_code, active')
          .eq('user_id', user.id)
          .maybeSingle()

        if (supplier?.id) {
          // Pré-remplit la wilaya si présente
          if (supplier.wilaya_code) {
            setWilayaCode(supplier.wilaya_code)
          }

          // Charge les documents du fournisseur
          const { data: docs } = await supabase
            .from('kyc_documents')
            .select('doc_type, status')
            .eq('entity_id', supplier.id).eq('entity_type', 'supplier')

          // Actuellement toujours modifiable (pas de status dans suppliers)
          setEditable(true)
          setNrcUploaded(
            docs?.some(
              (doc) => doc.doc_type === 'nrc' && doc.status !== 'rejected'
            ) ?? false
          )
        }
      }
